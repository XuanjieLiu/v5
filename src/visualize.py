...
from .plus_model import plus_once
...
def plot_heatmap(matrix: np.ndarray, output_path: Path, title: str) -> None:
...
def save_triplet_grid(
    ...
    images = batch["images"][:num_examples].to(v3_wrapper.device)
    numbers = batch["numbers"][:num_examples]
    _, emb_s, content_vq, _, _ = v3_wrapper.encode_and_quantize(images)
    _, pred_vq, _, _ = plus_once(plus_net, v3_wrapper, content_vq[:, 0], content_vq[:, 1])
...
def write_visuals(
    ...
    plot_heatmap(train_eval["heatmap"], vis_dir / "train_addition_heatmap.png", "Train addition accuracy")
    plot_heatmap(eval_eval["heatmap"], vis_dir / "eval_addition_heatmap.png", "Eval addition accuracy")
